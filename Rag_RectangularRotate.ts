...
export default class RAG_RectangularRotate extends hz.Component<typeof RAG_RectangularRotate> {
...
// Solo i lati che hai creato (E/W)
  ...
  __H_WALLS_SET2_ROT__: { type: hz.PropTypes.String, default: "WALL/DOOR2ROTATION" },
wallSet2YawOffset:    { type: hz.PropTypes.Number,  default: 90 }, // +90° = ruota oraria; usa -90 se serve
...
private genWall(
  ...
  // yaw base dal lato + offset extra (per SET 2)
  const yawBase = sideYaw(side, alongX);
  const yawDeg  = (yawBase + (yawExtra || 0) + 3600) % 360;
  ...
  // NB: inward rimane basato sul "side" (interno stanza). Per il tuo caso (solo rotazione visual/orientamento), funziona bene.
...
// === Walls + Doors — SET 2 (solo E/W come in scena) ===
{
  ...
  this.genWall(
    ...
    wantDoor2[s],
    rng,
    this.props.wallSet2YawOffset || 0 // <-- offset di yaw solo per SET 2
  );
}
...
hz.Component.register(RAG_RectangularRotate);
